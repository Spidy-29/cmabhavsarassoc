...
function promoslider_display_title($values){
	...
	switch( $display_title ){
		...
		case 'default': ?>
			<div class="slidercontent"><div class="<?php echo $title_classes; ?>"><?php
				if( !$disable_links ) echo '<a href="'.$destination_url.'" target="'.$target.'">';
				echo $title;
				if( !$disable_links ) echo '</a>'; ?>
			</div>
...
function promoslider_display_excerpt($values){
	...
	switch( $display_excerpt ){
		case 'excerpt': ?>
			<div class="promo_slider_excerpt"><?php echo $excerpt; ?></div><div class="knowmorebutton">Know More</div></div><?php
			break;
		default:
			return;
	}
